...
import { Num } from "@/components/site/Money";
...
function money(value: number | string, currency = "USD", locale = "en") {
  const n = typeof value === "string" ? Number(value) : value;
  if (!Number.isFinite(n)) return "—";
  return n.toLocaleString(locale === "ar" ? "ar-AE" : "en-US", { style: "currency", currency });
}
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container-px mx-auto max-w-[1100px] py-10">
        ...
        >
          <ArrowLeft className="h-4 w-4" /> {t("account.backToAccount")}
        </Link>

        {authLoading || orderQuery.isLoading ? (
          <p className="py-20 text-center text-sm text-muted-foreground">{t("account.loadingOrder")}</p>
        ) : !user ? (
          <div className="rounded-xl border border-border bg-white p-8 text-center">
            <p className="text-sm">{t("account.signInPrompt")}</p>
            <Button asChild className="mt-4"><Link to="/{-$lang}/account/login">{t("common.signIn")}</Link></Button>
          </div>
        ) : orderQuery.isError || !orderQuery.data ? (
          <div className="rounded-xl border border-border bg-white p-8 text-center">
            <h1 className="text-lg font-semibold">{t("account.orderUnavailable")}</h1>
            <p className="mt-2 text-sm text-muted-foreground">{t("account.orderUnavailableBody")}</p>
            <Button asChild variant="outline" className="mt-4"><Link to="/{-$lang}/account">{t("account.backToAccount")}</Link></Button>
          </div>
        ) : (
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-border bg-gradient-to-br from-primary to-blue-700 p-6 text-primary-foreground sm:p-8">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-wider text-white/70">{t("account.orderNumber")}</p>
            <h1 className="text-2xl font-bold sm:text-3xl"><Num>#{order.number}</Num></h1>
            <p className="mt-1 text-sm text-white/80">
              {t("account.placedOn")}{" "}
              <bdi dir="ltr">{new Date(order.date_created).toLocaleDateString(locale === "ar" ? "ar-AE" : undefined, { year: "numeric", month: "long", day: "numeric" })}</bdi>
            </p>
          </div>
          <div className="text-end">
            ...
            <bdi dir="ltr" className="mt-2 block text-2xl font-bold">{money(order.total, currency, locale)}</bdi>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-white p-6">
        <div className="mb-4 flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold">{t("account.itemsCount", { count: order.line_items.length })}</h2>
        </div>
          ...
          {order.line_items.map((item) => (
            <div key={item.id} className="flex items-center gap-4 py-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-xs text-muted-foreground">{t("product.quantity")}: <Num>{item.quantity}</Num></p>
              </div>
              <bdi dir="ltr" className="text-sm font-semibold">{money(item.total, currency, locale)}</bdi>
            </div>
          ))}
        </div>
        <div className="mt-4 flex justify-end border-t border-border pt-4">
          <div className="w-full max-w-xs space-y-1 text-sm">
            <div className="flex justify-between text-base font-bold">
              ...
              <bdi dir="ltr">{money(order.total, currency, locale)}</bdi>
            </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <AddressBlock title={t("account.billingAddress")} addr={order.billing} />
        <AddressBlock title={t("account.shippingAddress")} addr={order.shipping} />
      </div>
    </div>
  );
}
