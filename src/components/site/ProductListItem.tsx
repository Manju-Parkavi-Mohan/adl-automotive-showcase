  ...
  return (
    <article className="group grid grid-cols-1 gap-6 overflow-hidden rounded-xl border border-border bg-card p-5 transition-shadow hover:shadow-[var(--shadow-card)] sm:grid-cols-[220px_1fr_auto]">
      <div className="relative aspect-square overflow-hidden rounded-lg bg-secondary">
        ...
        />
        {product.badge === "new" && (
          <span className="absolute left-3 top-3 rounded-full bg-[var(--accent-blue)] px-2.5 py-0.5 text-xs font-semibold text-white">
            NEW
          </span>
        )}
        {discount > 0 && (
          <span className="absolute left-3 top-3 rounded-full bg-destructive px-2.5 py-0.5 text-xs font-semibold text-white">
            -{discount}%
          </span>
        )}
      </div>
      ...
      <div className="flex flex-row items-end justify-between gap-3 sm:flex-col sm:items-end sm:justify-start">
        ...
        <div className="flex flex-col gap-2 sm:w-44">
          ...
          <div className="flex gap-2">
            <button className="grid h-9 flex-1 place-items-center rounded-md border border-border transition-colors hover:border-primary hover:text-primary" aria-label="Quick view">
              <Eye className="h-4 w-4" />
            </button>
            <button className="grid h-9 flex-1 place-items-center rounded-md border border-border transition-colors hover:border-primary hover:text-primary" aria-label="Wishlist">
